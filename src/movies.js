...
function howManyMovies(moviesArray) {
    let dramaStevSpielMovies =moviesArray.filter((movie)=>{return movie.director==="Steven Spielberg" && movie.genre.includes("Drama")===true})
...
function scoresAverage(moviesArray) {
    ...
    let score = moviesArray.reduce((totalScore,movie)=>{
      if (movie.score===undefined){return totalScore}
      else{return totalScore+movie.score}
    },0)
    let averageScore = score/moviesArray.length
